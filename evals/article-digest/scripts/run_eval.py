#!/usr/bin/env python3
"""Run the article-digest arms over the URL subset via `hiveloom run --json`.

Resumable: (arm, sample, epoch) triples already present in the results file
are skipped, so an interrupted sweep can simply be re-run.

Usage:
    python scripts/run_eval.py [--epochs 2] [--arms opus-harness,opus-raw,...]
"""
...
# Article-like pages from the shared dataset. Excluded: s20 (known heading
# drift), s32 (deliberate 404), and the reference-doc pages (s09-s16, s31),
# which are not digest-shaped. The probe default uses the first 5; pass
# --samples 10 for the full list.
SAMPLE_IDS = ["s01", "s07", "s18", "s25", "s29", "s03", "s17", "s22", "s23", "s28"]
...
def main() -> None:
    ...
    ap.add_argument("--samples", type=int, default=5, help="how many URLs (max 10)")
    ap.add_argument("--arms", default=",".join(ARMS))
    ap.add_argument("--out", default="results/results.jsonl")
    args = ap.parse_args()

    # A single eval-level .env is enough: export its vars to every arm run.
    env_file = ROOT / ".env"
    if env_file.exists():
        import os
