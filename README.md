...
Four worked examples live in [`harnesses/`](https://github.com/FrancescoMrn/hiveloom/tree/main/harnesses),
...
They are generated by `scripts/build_harnesses.py`, which builds each one from
zero through the same `init`/`add`/`set` calls the CLI exposes. Nothing there
writes `harness.yaml` directly, so the demos cannot drift away from what the
