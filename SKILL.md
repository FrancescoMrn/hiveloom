...
2. **Construct incrementally** — each step is validated and rolled back on error:
   ...
   # `set`/`remove` paths may index into an existing list item, e.g. the
   # cost guardrail `init` seeds at guardrails.0 (no append via `set`):
   hiveloom set guardrails.0.value 0.05 --dir ./h
   # add mcp-server: --timeout-seconds (default 30, 0 < t <= 600) covers
   # connect/initialize + each tool call; raise it for a slow peer harness:
   hiveloom add mcp-server --name jira --url https://mcp.acme.example/mcp --timeout-seconds 120 --dir ./h
   ```
   A `set` value parses as YAML, except a plain string field (e.g.
   `system_prompt`) keeps the CLI text verbatim so `": "` isn't misread as a
   mapping; use `--file` when that heuristic can't resolve the field.
...
`evolution.reflect`, `evolution.trace_excerpts`, `evolution.objectives`, or the `memory` budgets
