...
- [harnesses/](harnesses/) — four worked examples to imitate: `quickstart`
  (no tools), `example-summarizer` (tools + verification),
  `article-extractor` (a custom tool + anti-hallucination validator), and
  `routing-lab` (playbooks, forking, evolution — offline, no API key).
  They are generated by `scripts/build_harnesses.py`; change that script
  and rebuild rather than editing a demo's `harness.yaml`.
